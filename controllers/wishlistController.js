...
const updateWishlist = async (req, res) => {
    try {
        ...
        const foundWishlist = await WishlistModel.find({_id:{$ne:WishlistId},UserId:reqData.UserId,Name:reqData.Name});
        ...
        else
        {
            ...
            const updatedWishlist = await WishlistModel.findByIdAndUpdate(WishlistId,{
...
const addWishlist = async (req, res) => {
    try{
        ...
        const foundWishlist = await WishlistModel.find({UserId:reqData.UserId,Name:reqData.Name});
        console.log(foundWishlist.length);
        if(foundWishlist.length > 0)
        {
            return res.status(400).json({
